...
#'   What fraction of the data should be used for early stopping (i.e. as a training set). This is apparently just the holdout split for optimization.
#'   Default is \code{4/5}.
...
#' @export
autoxgboost_space = function(task, predict.type = "response",
...
#    add_dep("xgboost.max_leaves", "xgboost.grow_policy", CondEqual$new("lossguide"))
  ...
  autoxgbparset$trafo = trafo
  autoxgbparset.mixed$trafo = trafo
  ...
  list(learner = GraphLearner$new(graph, predict_type = predict.type),
    searchspace = autoxgbparset, searchspace.mixed = autoxgbparset.mixed,
    resampling = rsmp("holdout", ratio = early.stopping.fraction)$instantiate(task))
}
